...
import { Trophy, RefreshCw, Filter, ShieldCheck } from "lucide-react";
import { toast } from "sonner";

export default function StudentStenoLeaderboardPage() {
    ...
    <div className="space-y-6 p-1 sm:p-2">
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <Trophy className="w-6 h-6 text-amber-500" /> Steno Student Leaderboard
          </h1>
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Compare transcription speed, accuracy, and overall scores with fellow stenography learners.
          </p>
        </div>
      ...
      <Card className="p-4 rounded-3xl border-slate-200 bg-white shadow-xs space-y-3">
        <div className="flex items-center gap-1.5 text-xs font-black uppercase tracking-wider text-slate-400">
          <Filter className="w-4 h-4 text-indigo-600" /> Filter Leaderboard
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase text-slate-400">Speed</label>
            <select
              value={targetWpm || ""}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase text-slate-400">Series</label>
            <select
              value={seriesId}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase text-slate-400">Passage</label>
            <select
              value={passageId}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase text-slate-400">Timeframe</label>
            <select
              value={dateRange}
      ...
      {loading ? (
        ...
        <Card className="py-16 text-center text-slate-400 rounded-3xl border-dashed">
          No rankings found for the selected filter parameters.
        </Card>
      ) : (
