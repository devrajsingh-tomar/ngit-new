...
import { 
    ChevronLeft, Save, Plus, Trash2, Info, Loader2 
} from "lucide-react";
...
export default function EditQuestionPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const init = async () => {
            await Promise.all([loadCourses(), loadQuestion()]);
            setLoading(false);
        };
        init();
    }, [id]);
        ...
        <div className="max-w-4xl mx-auto py-12 px-6 animate-in fade-in duration-500 pb-24">
            ...
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-medium text-slate-500">Modify question ID: {id.slice(-6).toUpperCase()}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 space-y-10">
                    ...
                    {formData.type === "DESCRIPTIVE" && (
                        <div className="bg-indigo-50/80 p-6 rounded-2xl border border-indigo-100 flex gap-4 items-start">
                            ...
                            <div>
                                ...
                                <p className="text-indigo-700/80 font-medium mt-1">Manual evaluation required.</p>
                            </div>
                        </div>
                    )}
                    ...
                    {formData.type === "ASSERTION_REASON" && (
                        <div className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <Label className="font-bold text-indigo-700 ml-1">Assertion (A)</Label>
                                    <Textarea 
                                        className="min-h-[120px] rounded-[1.5rem] bg-indigo-50/50 border border-indigo-100 p-5 font-medium"
                                        value={formData.assertion?.en || ""}
                                        onChange={(e) => setFormData({...formData, assertion: {en: e.target.value}})}
                                    />
                                </div>
                                <div className="space-y-2">
                                    <Label className="font-bold text-purple-700 ml-1">Reason (R)</Label>
                                    <Textarea 
                                        className="min-h-[120px] rounded-[1.5rem] bg-purple-50/50 border border-purple-100 p-5 font-medium"
                            ...
                            <div className="space-y-3">
                                <Label className="font-bold text-slate-700 ml-1">Correct Outcome</Label>
                                <div className="space-y-2">
                                    {[
                                        ...
                                        <div 
                                            ...
                                            className={`p-4 rounded-2xl border-2 cursor-pointer transition-all font-bold text-sm flex items-center gap-3 ${
                                                Number(formData.numericAnswer) === i ? "border-emerald-500 bg-emerald-50" : "border-slate-100 bg-white"
                                            }`}
                                        >
                                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${Number(formData.numericAnswer) === i ? "border-emerald-500 bg-emerald-500" : "border-slate-300"}`} />
                                            {opt}
                                        </div>
                                    ))}
                                </div>
                ...
                <div className="grid grid-cols-2 gap-6 pt-4">
                    <div className="space-y-2">
                        <Label className="font-bold text-slate-700 ml-1">Marks (+)</Label>
                        <Input 
                            type="number"
                            className="h-14 rounded-2xl bg-slate-50 border-none px-5 font-black text-emerald-600 text-xl"
                    ...
                    <div className="space-y-2">
                        ...
                        <Input 
                            type="number"
                            className="h-14 rounded-2xl bg-slate-50 border-none px-5 font-black text-rose-500 text-xl"
                ...
                <div className="space-y-3 border-t border-slate-100 pt-10">
                    ...
                    <Textarea 
                        ...
                        className="min-h-[160px] rounded-[1.5rem] bg-slate-50 border-none p-6 font-medium text-[15px]"
