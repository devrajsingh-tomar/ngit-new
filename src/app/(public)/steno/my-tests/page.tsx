...
import { Clock, Eye, RefreshCw, Award, Plus, Trash2, Play, RotateCcw } from "lucide-react";
import { toast } from "sonner";

export default function StenoMyTestsPage() {
  ...
  const loadData = async () => {
    ...
    if (historyRes.success && historyRes.results) {
      setResults(historyRes.results);
    }
    if (customRes.success && customRes.customTests) {
      setCustomTests(customRes.customTests);
    }
  ...
  return (
    <div className="bg-slate-50 min-h-screen p-4 sm:p-6 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <Clock className="w-6 h-6 text-purple-600" /> My Steno Tests & History
          </h1>
      ...
      {/* Section 1: My Custom Steno Tests (Step 15) */}
            ...
            {customTests.map((t) => (
              <Card key={t._id} className="p-5 rounded-3xl border-slate-200 bg-white shadow-xs space-y-4">
                <div className="flex justify-between items-start">
                  <span className="text-[10px] font-black uppercase tracking-wider bg-purple-50 text-purple-700 px-2.5 py-1 rounded-md border border-purple-100">
                ...
                <div className="flex gap-2 pt-2">
                  <Link href={`/steno/passage/${t.passageId?._id || t.passageId}`} className="flex-1">
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <Card className="p-8 text-center text-slate-400 rounded-3xl border-dashed bg-white">
            No completed attempts yet. Practice dictations to view your historical reports here.
          </Card>
        ) : (
            ...
            {results.map((r) => (
              <Card key={r._id} className="p-5 rounded-2xl border-slate-200 bg-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-purple-300 transition-all">
                ...
                <div className="flex gap-2">
                  ...
                  <Link href={`/steno/transcription/${r._id}`}>
                    <Button size="sm" className="bg-purple-600 hover:bg-purple-700 text-white font-bold h-8 text-xs rounded-xl gap-1">
      ...
      {/* Modal Dialog: Create Custom Steno Test (Step 15) */}
        ...
        <DialogContent className="max-w-xl rounded-3xl p-6">
          ...
          <form onSubmit={handleCreateCustomTest} className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700">Hindi Font (Step 14)</label>
                <select
