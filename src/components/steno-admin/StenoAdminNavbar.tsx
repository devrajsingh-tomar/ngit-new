...
interface StenoAdminNavbarProps {
    onMenuClick?: () => void;
}
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <button className="flex items-center gap-4 hover:bg-slate-50/80 pl-1.5 pr-3 py-1.5 rounded-2xl transition-all group border border-transparent hover:border-slate-100 outline-none">
                                <div className="text-right hidden sm:block">
                                    <p className="text-sm font-black text-slate-900 tracking-tight">{session?.user?.name || "NGIT Steno Institute Admin"}</p>
                                    <p className="text-[10px] text-indigo-600 font-black uppercase tracking-[0.1em] mt-0.5">STENO ADMIN WORKSPACE</p>
                                </div>
                                <div className="relative">
                        ...
                        <DropdownMenuContent className="w-64 rounded-2xl p-2 shadow-2xl border-slate-100" align="end" sideOffset={8}>
                            ...
                            <DropdownMenuItem 
                                ...
                                onClick={() => signOut({ callbackUrl: '/steno/login' })}
                            >
